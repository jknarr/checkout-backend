...
class AuthSessionServiceTest {
    ...
    void setUp() {
        ...
        service = new AuthSessionService(userRepository, cardRepository, shippingAddressRepository,
                authSessionRepository, sessionRepository, otpService, authTokenService, deviceKeyService, passkeyService,
                bcrypt, new ObjectMapper());
        Session checkoutSession = Session.builder()
                .id(checkoutSessionId)
                .merchantId("demo-merchant-001")
                .status(SessionStatus.PENDING)
                .cartJson("[{\"productId\":\"sku-1\",\"name\":\"Item\",\"price\":10.0,\"quantity\":1}]")
                .expiresAt(Instant.now().plusSeconds(1800))
                .build();
        lenient().when(sessionRepository.findById(any())).thenReturn(Optional.of(checkoutSession));
    ...
    void handleAction_passkeyAuthCancel_returnsOtpVerifyStep() {
        ...
        when(authSessionRepository.save(any())).thenAnswer(inv -> inv.getArgument(0));
        when(otpService.isMockMode()).thenReturn(true);
        when(otpService.createChallenge(testUser.getPhoneNumber())).thenReturn(
                OtpChallenge.builder()
                        .id(challengeId)
        ...
        assertEquals("654321", response.otpCode());
    }
}
