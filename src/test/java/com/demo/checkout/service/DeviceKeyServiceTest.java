...
import com.demo.checkout.exception.DeviceVerificationException;
import com.demo.checkout.repository.DeviceRegistrationRepository;
...
class DeviceKeyServiceTest {
    ...
    void registerDevice_success_returnsDeviceId() {
        ...
        when(deviceRegistrationRepository.findByUserIdAndPublicKeyJwk(userId, "{}")).thenReturn(Optional.empty());
    ...
    @Test
    void registerDevice_existingDevice_returnsExistingDeviceId() {
        UUID existingDeviceId = UUID.randomUUID();
        DeviceRegistration existing = DeviceRegistration.builder().id(existingDeviceId).user(testUser)
                .publicKeyJwk("{\"new\":true}").build();
        when(userRepository.findById(userId)).thenReturn(Optional.of(testUser));
        when(deviceRegistrationRepository.findByUserIdAndPublicKeyJwk(userId, "{\"new\":true}"))
                .thenReturn(Optional.of(existing));

        UUID result = service.registerDevice(userId, "{\"new\":true}");

        assertEquals(existingDeviceId, result);
        verify(deviceRegistrationRepository, never()).save(any());
    }
