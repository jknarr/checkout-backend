...
import com.demo.checkout.domain.User;
import com.demo.checkout.exception.PasskeyException;
import com.demo.checkout.repository.PasskeyCredentialRepository;
import com.demo.checkout.repository.UserRepository;
...
public class PasskeyService {
    ...
    @Value("${demo.passkey.rp-id}")
    private String rpId;

    @Value("${demo.passkey.rp-name}")
    private String rpName;

    @Value("${demo.passkey.origin}")
    private String origin;

    @Value("${demo.passkey.challenge-ttl-seconds:120}")
    ...
    public JsonNode beginRegistration(UUID authSessionId, User user) {
        try {
            ...
            // Keep full options JSON for server-side verification replay.
            String serverJson = options.toJson();
            // Send browser-ready JSON shape to SDK for navigator.credentials.create().
            String clientJson = options.toCredentialsCreateJson();

            challengeStore.put(authSessionId, user.getId(), "registration", serverJson,
            ...
            return objectMapper.readTree(clientJson);
        } catch (Exception e) {
    ...
    public void finishRegistration(UUID authSessionId, User user, String clientResponseJson) {
        ...
        try {
            ...
            String aaguid = null;
            Object aaguidValue = result.getAaguid();
            if (aaguidValue instanceof ByteArray byteArray) {
                aaguid = byteArray.getHex();
            } else if (aaguidValue instanceof Optional<?> optional && optional.isPresent()) {
                Object nestedValue = optional.get();
                if (nestedValue instanceof ByteArray byteArray) {
                    aaguid = byteArray.getHex();
                }
            }
    ...
    public JsonNode beginAssertion(UUID authSessionId, User user) {
        try {
            ...
            // Keep full assertion request for server-side finishAssertion parsing.
            String serverJson = request.toJson();
            // Send browser-ready JSON shape to SDK for navigator.credentials.get().
            String clientJson = request.toCredentialsGetJson();

            challengeStore.put(authSessionId, user.getId(), "assertion", serverJson,
                    Instant.now().plusSeconds(challengeTtlSeconds));
