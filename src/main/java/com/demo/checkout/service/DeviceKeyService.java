...
public class DeviceKeyService {
    ...
    @Value("${demo.device.challenge-expiry-seconds}")
    ...
    public UUID registerDevice(UUID userId, String publicKeyJwk) {
        ...
        Optional<DeviceRegistration> existing = deviceRegistrationRepository.findByUserIdAndPublicKeyJwk(userId, publicKeyJwk);
        if (existing.isPresent()) {
            log.info("Device already registered for userId: {}, deviceId: {}", userId, existing.get().getId());
